...
BN128.promise = mcl.init(mcl.BN_SNARK1).then(() => {
    ...
    const PPLUS1DIV4 = toBytes("0x0c19139cb84c680a6e14116da060561765e05aa45a1c72a34f082305b61f3f52"); // big-endian.
    ...
    BN128.BASE = BN128.mapInto(keccak256("g"));
    ...
    BN128.sign = (address, secret) => { // i guess `message` is a string and `secret` is a uint8array...
      ...
      const c = new mcl.Fr();
      c.setBigEndianMod(toBytes(keccak256(encodeAbiParameters([
        { name: "", type: "string" },
        { name: "", type: "address" },
        { name: "", type: "bytes32" },
        { name: "", type: "bytes32[2]" },
    ...
    BN128.gEpoch = (epoch) => {
      return BN128.mapInto(keccak256(encodePacked(["string", "uint256"], ["Firn Epoch", epoch])));
    };
  }
);
