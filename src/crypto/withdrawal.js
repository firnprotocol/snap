...
export class WithdrawalProof {
  ...
  static prove(Y, Cn, C, epoch, sk, r, bTransfer, bDiff, index, fee, destination, data) {
    const result = new WithdrawalProof();

    const D = C.vector[0].right;
    const pub = Y.vector[index];
    const aL = new FieldVector(Array.from({ length: M }).map((_, i) => {
    ...
    const sL = new FieldVector(Array.from({ length: M }).map(BN128.randomScalar));
    const sR = new FieldVector(Array.from({ length: M }).map(BN128.randomScalar));
    result.BS = PedersenVectorCommitment.commit(sL, sR);

    const a = new FieldVector(Array.from({ length: n }).map(BN128.randomScalar));
    const b = new FieldVector(Array.from({ length: n }).map((_, i) => {
      const bit = new mcl.Fr();
      bit.setInt(index >> i & 0x01);
      return bit;
    }));
    const c = new FieldVector(a.vector.map((elem, i) => {
    ...
    for (let i = 1; i < N; i++) { // it would be nice to have a nifty functional way of doing this.
      vPow.push(mcl.mul(vPow.vector[i - 1], v));
    }

    const polyL = recursivePolynomials([new Polynomial([BN128.ONE])], a.vector.slice(), b.vector.slice());
    const poly = Array.from({ length: n }).map((_, k) => new FieldVector(polyL.map((P_i) => P_i.coefficients[k])));

    const Phi = Array.from({ length: n }).map(() => ElGamal.commit(pub));

    result.CnG = Array.from({ length: n }).map((_, k) => Cn.multiExponentiate(poly[k]).add(Phi[k]));
    ...
    result.C_XG = Array.from({ length: n }).map((_, k) => {
      ...
      return result.plus(mcl.mul(mcl.sub(vPow.vector[index], BN128.ONE), mcl.mul(poly[k].vector[index], mcl.add(bTransferMCL, feeMCL))));
    });
    ...
    result.tHat = mcl.add(mcl.add(tPolyCoefficients[0], mcl.mul(tPolyCoefficients[1], x)), mcl.mul(tPolyCoefficients[2], mcl.sqr(x)));
    const tauX = mcl.add(mcl.mul(result.T_1.randomness, x), mcl.mul(result.T_2.randomness, mcl.sqr(x)));
    ...
    y_XR = mcl.add(y_XR, Y.multiExponentiate(p.add(p.negate().plus(wPow).hadamard(vPow)))); // only need the RHS
    ...
    const A_b = mcl.add(mcl.mul(ElGamal.base.g, k_b), mcl.mul(CRnR, mcl.mul(zs[0], k_sk))); // el gamal???
    const A_X = mcl.mul(y_XR, k_r); // y_XR.mul(k_r);
    const A_t = mcl.add(mcl.mul(PedersenCommitment.base.g, mcl.neg(k_b)), mcl.mul(PedersenCommitment.base.h, k_tau));
    ...
    result.s_b = mcl.add(k_b, mcl.mul(result.c, mcl.mul(mcl.mul(bDiffMCL, zs[0]), wPow)));
