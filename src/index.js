...
export const onRpcRequest = async ({ origin, request }) => {
  switch (request.method) {
    ...
    case "requestBalance": {
      ...
      if (name === undefined) throw new Error(`The chain ID ${chainId} is not supported by Firn.`);
    ...
    case "transact": {
      ...
      if (state === null || !(address in state)) throw new Error("User has not yet logged in under this address.");
      const plaintext = state[address];
      await algebra;
      const secret = new mcl.Fr();
      secret.setBigEndianMod(toBytes(plaintext));
      ...
      const calculators = {
        ...
        Ethereum: async (l1Gas, maxPriorityFeePerGas) => {
          const feeHistory = await publicClient.getFeeHistory({ blockCount: 1, rewardPercentiles: [] });
        ...
        "OP Mainnet": async (l2Gas, txCompressedSize) => {
          ...
          const { maxFeePerGas: l2GasPrice } = await publicClient.estimateFeesPerGas();
